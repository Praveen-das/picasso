...
export default function ProfileCredentialForm() {
    ...
    const actionExpiryDate = useStore(state => state.userData?.actionExpiryDate)
    ...
    useEffect(() => {
        ...
        if (currentTime > actionExpiryDate)
            return setDateLockExpired(true)
        ...
        switch (type) {
            case 'displayname':
                updateDisplayName(
                    ...
                    type)
                    .then(() => resetValues())
                    .catch(error => setError(handleExceptions(error)))
                break;
            case 'email':
                updateUserEmail(email.current.value)
                    .then(() => resetValues())
                    .catch(error => setError(handleExceptions(error)))
                break;
            case 'password':
    ...
    const style = {
        title: {
            variant: 'h6', color: 'primary', fontSize: 14, fontWeight: 600
        },
        button: {
            sx: { justifySelf: 'flex-end' }
        },
        summery: {
            variant: 'h6', fontSize: 15, fontWeight: 500, color: '#777'
        },
        details: {
                ...
                <div>
                            ...
                            false ?
                                <AccordionDetails sx={{ mb: -4 }}>
                                    <div className='accordion_warning warning'>
                                        <Typography {...style.details}>{`You can not change your name for ${remainingTime && remainingTime}`}</Typography>
                                    </div>
                                </AccordionDetails> :
                                <AccordionDetails>
                                    {/* WARNIG: If you change your name, you won't be able to change it again for 60 days.  */}
                                    {
                                        (error?.fName || error?.textField) && <AlertBox message={error?.fName || error?.textField} />
                                    }
                            ...
                            <div id='accordion_style'>
                                ...
                                <>
                                    <Typography
                                        ...
                                        {...style.summery}>
                                        {user?.uid}
                                        <IconButton onClick={() => copyToClipboard('User Id', user?.uid)}>
                    ...
                    <Accordion expanded={expanded === 'panel2'}>
                                    ...
                                    expanded === 'panel2' ?
                                        ...
                                        <>
                                            <Typography
                                                {...style.summery} >
                                                {user?.email}
                                                <IconButton onClick={() => copyToClipboard('Email', user?.email)}>
                                                    <ContentCopyIcon sx={{ fontSize: 18 }} />
                                                </IconButton>
                                            </Typography>
                        ...
                        <AccordionDetails>
                            ...
                            {
                                (error?.email || error?.textField) && <AlertBox message={error?.email || error?.textField} />
                            }
                            <Grid container spacing={4} sx={{ mt: -5 }}>
                    ...
                    <Accordion expanded={expanded === 'panel3'}>
                        ...
                        <AccordionDetails>
                            ...
                            {
                                (error?.password || error?.newPass || error?.confirmPass || error?.textField) && <AlertBox message={error?.password || error?.newPass || error?.confirmPass || error?.textField} />
                            }
                            <Grid container spacing={4}>
                                <Grid item {...MQ.rows_3}>
                                    <TextField
                                        type='password'
                                        error={(error?.newPass || error?.password || error?.textField) !== undefined}
                                        inputRef={password}
                                        label='New Password'
                                        {...style.textField} />
                                </Grid>
                                <Grid item {...MQ.rows_3}>
                                    <TextField
                                        type='password'
                                        error={(error?.confirmPass || error?.password || error?.textField) !== undefined}
