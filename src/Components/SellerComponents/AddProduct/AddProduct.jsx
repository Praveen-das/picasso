...
function AddProduct({ setModel, model, _product }) {
  ...
  function handleSubmit(obj) {
    if (model === "add") return _addProduct(obj).then(() => setModel(false));
    _updateProduct({ id: _product.id, updates: obj }).then(() =>
      setModel(false)
    );
  }
      ...
      <ThemeProvider theme={theme}>
        ...
        >
          <Slide direction="left" in={open} mountOnEnter unmountOnExit>
            <Box sx={box_style}>
              <Formik
                initialValues={model === "add" ? initialValues : initialValues2}
                validationSchema={
